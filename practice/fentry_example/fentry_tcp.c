...
static int handle_event(void *ctx, void *data, size_t data_sz)
{
	...
	printf("PID=%u COMM=%-16s 请求=%llu 返回=%d %s:%u -> %s:%u\n", e->pid, e->comm,
	       (unsigned long long)e->bytes_sent, e->return_value, saddr, e->sport, daddr,
	       e->dport);
	return 0;
}
...
int main(void)
{
	struct fentry_tcp_bpf *skel = NULL;
	...
	skel = fentry_tcp_bpf__open();
	if (!skel) {
		fprintf(stderr, "打开 skeleton 失败\n");
		return 1;
	}

	err = fentry_tcp_bpf__load(skel);
	if (err) {
		fprintf(stderr, "加载 BPF 对象失败: %d\n", err);
		goto cleanup;
	}

	err = fentry_tcp_bpf__attach(skel);
	if (err) {
		fprintf(stderr, "附加 fentry/fexit 程序失败: %d\n", err);
		goto cleanup;
	}
	...
	printf("开始跟踪 tcp_sendmsg，按 Ctrl+C 退出...\n");
	while (!exiting) {
